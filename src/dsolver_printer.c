 ...
 */
void dsolver_splx_print_solution(FILE *f, mini_simplex_t *s) {
  uint32_t i, n, m;
 ...
 */
void dsolver_splx_print_bounds(FILE *f, mini_simplex_t *s) {
  uint32_t i, n;
  ...
  if (n > 0) {
    fputs("--- Bounds ---\n", f);
    for (i=1; i<n; i++) {
      if (mini_simplex_var_has_bounds(s, i)) {
	fputs("  ", f);
	if (mini_simplex_var_has_lb(s, i)) {
	  q_print(f, s->lb + i);
	  fputs(" <= ", f);
	}
	dsolver_print_var(f, i, n);
	if (mini_simplex_var_has_ub(s, i)) {
	  fputs(" <= ", f);
	  q_print(f, s->ub + i);
	}
	fputc('\n', f);
 ...
 */
void dsolver_splx_print_sol_and_bounds(FILE *f, mini_simplex_t *s) {
  uint32_t i, n, m;
  ...
  if (n > 0) {
    fputs("--- Assignment and bounds ---\n", f);
    for (i=1; i<n; i++) {
      fputs("  val[", f);
      dsolver_print_var(f, i, m);
      fputs("] = ", f);
      q_print(f, s->val + i);
      if (i < m && mini_simplex_var_has_bounds(s, i)) {
	fputs("       ", f);
	if (mini_simplex_var_has_lb(s, i)) {
	  q_print(f, s->lb + i);
	  fputs(" <= ", f);
 ...
 */
void dsolver_splx_print_status(FILE *f, mini_simplex_t *s) {
  row_t *row;
  uint32_t i, n;
  int32_t x, y, r;

  switch (s->status) {
  case MINI_SIMPLEX_READY:
    fputs("--- Status: ready ---\n", f);
    break;
  case MINI_SIMPLEX_UNKNOWN:    
    fputs("--- Status: unknown ---\n", f);
    dsolver_splx_print_infeasible_vars(f, s);
    break;
  case MINI_SIMPLEX_SAT:
    fputs("--- Status: sat ---\n", f);
    break;
  case MINI_SIMPLEX_BOUND_CONFLICT:
    fputs("--- Status: bound conflict ---\n", f);
    x = s->conflict_var;
    fputs("Conflict var = ", f);
    dsolver_print_var(f, x, s->nvars);
    fputs(", lower bound = ", f);
    q_print(f, s->lb + x);
    fputs(", upped boun = ", f);
    q_print(f, s->ub + x);
  ...
  case MINI_SIMPLEX_ROW_CONFLICT:
    fputs("--- Status: row conflict ---\n", f);
    x = s->conflict_var;
    fputs("Conflict var = ", f);
    dsolver_print_var(f, x, s->nvars);
    fputs(", val = ", f);
    ...
    fputs("Conflict row: ", f);
    r = matrix_basic_row(&s->matrix, x);
    row = matrix_row(&s->matrix, r);
    dsolver_print_matrix_row(f, row, s->nvars);
    fputs("\nBounds:\n", f);
    n = row->size;
    for (i=0; i<n; i++) {
      y = row->data[i].c_idx;
      if (y >= 1) {
	fputs("  ", f);
	if (mini_simplex_var_has_lb(s, y)) {
	  q_print(f, s->lb + y);
	  fputs(" <= ", f);
	}
	dsolver_print_var(f, y, s->nvars);
	if (mini_simplex_var_has_ub(s, y)) {
	  fputs(" <= ", f);
	  q_print(f, s->ub + y);
	}
	fputc('\n', f);
 ...
 */
void dsolver_splx_print_infeasible_vars(FILE *f, mini_simplex_t *s) {
  uint32_t i, n, m;
  int32_t *h;

  m = s->nvars;
  n = int_heap_nelems(&s->infeasible_vars);
  h = s->infeasible_vars.heap;

  if (n == 0) {
    fputs("Infeasible vars: []\n", f);
  } else {
    fputs("Infeasible vars: [", f);
    dsolver_print_var(f, h[1], m);
    for (i=2; i<=n; i++) {
      fputc(' ', f);
      dsolver_print_var(f, h[i], m);
    }
    fputs("]\n", f);
  }
}
...
/*
 * Print x from simplex solver s
 */
void dsolver_splx_print_var(FILE *f, mini_simplex_t *s, int32_t x) {
  dsolver_print_var(f, x, s->nvars);
}
