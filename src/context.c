 ...
 * - x must be the code of a bitvector variable v, so we return v.
 */
static inline thvar_t translate_code_to_bv(context_t *ctx, int32_t x) {
  thvar_t v;
 ...
 * - if x may be the code for true_occ or false_occ
...
static const th_ctrl_interface_t null_ctrl = {
...
static const th_smt_interface_t null_smt = {
 ...
 * - attach it to the core and initialize the core
 */
...
// valid arch: only bitvector or nosolvers
...
int32_t assert_formulas(context_t *ctx, uint32_t n, term_t *f) {
  ...
  assert(smt_status(ctx->core) == STATUS_IDLE);
