...
void reset_bvexp_table(bvexp_table_t *table) {
  ...
  /*
   * aux buffers must be deleted first since their
   * content may become invalid pointers afer the reset_objstore
   * calls. Just calling _prepare is not enough as it keeps
   * the end_marker in table->aux/table->aux64.
   */
  ...
  // Now we must recreate the buffers aux/aux64
  init_bvarith_buffer(&table->aux, &table->pprods, &table->store);
  init_bvarith64_buffer(&table->aux64, &table->pprods, &table->store64);
}
