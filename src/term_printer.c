...
static void print_named_pprod(FILE *f, term_table_t *tbl, pprod_t *r) {
  if (pp_is_var(r)) {
    print_term_name(f, tbl, var_of_pp(r));
  } else if (pp_is_empty(r)) {
    fputc('1', f);
  } else if (r == end_pp) {
    fputs("end_pp", f);
...
void print_term_table(FILE *f, term_table_t *tbl) {
  ...
  for (i=0; i<n; i++) {
	...
	break;
      ...
      case ARITH_EQ_ATOM:
	fputs("(arith-eq ", f);
	print_term_name(f, tbl, tbl->desc[i].integer);
	fputs(" 0)", f);
	break;
