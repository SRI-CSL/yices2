...
/*
 * An internalization table keeps track of variable substitutions
 * and of the internal object (egraph term or other) mapped to 
 * terms. It stores a partition of term indices in a union-find data 
 * structure:
 * - All the elements of an equivalence class are equal and 
 *   mapped to the same solver object.
 * - Each class contains a distinguished root element. All elements
 *   other than the root are uninterpreted term indices (i.e., variables).
 *
 ...
 */
static inline bool intern_tbl_root_is_free(intern_tbl_t *tbl, term_t t) {
  assert(intern_tbl_is_root(tbl, t));
  return au8_read(&tbl->rank, index_of(t)) < 255;
}
...
/*
 * Check whether the substitution [r1 := r2] where r2 is a constant
 * is valid. This does the same checks as the previous function and 
 * also checks whether r2's type is a subtype of r1's type.
 *
