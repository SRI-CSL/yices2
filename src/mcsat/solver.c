...
//          fprintf(stderr, "skipping propagation (bool)\n");
  ...
  if (mcsat_conflict_with_assumptions(mcsat, conflict_level)) {
    ...
    if (mcsat->ctx->mcsat_options.minimize_lemmas) {
      ...
      // Minimization only removes redundant disjuncts; it never removes the
      // asserting disjunct, so the UIP invariant (exactly one top-level
      // variable) must still hold.
      assert(conflict_get_top_level_vars_count(&conflict) == 1);
    }
