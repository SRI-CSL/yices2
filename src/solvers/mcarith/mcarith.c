...
struct mcarith_solver_s {
  ...
  // Vector for storing assertions mcsat sees.
  // Each element is a term formed 
  ivector_t mcsat_assertions;
...
#define DEF_MCSAT_VAR_VECTOR_SIZE 64
#define DEF_MCSAT_UNDO_STACK_SIZE 16
#define DEF_MCSAT_ASSERTION_VECTOR_SIZE 32
...
void init_mcarith_solver(mcarith_solver_t **solver, context_t* ctx) {
  ...
  init_ivector(&s->var_terms, s->simplex.vtbl.size);
  rational_t one;
  q_init(&one);
  q_set_one(&one);
  ivector_push(&s->var_terms, arith_constant(mcarith_solver_terms(s), &one));
  ...
  // Create undo stack
  init_mcarith_undo_stack(&s->undo_stack, DEF_MCSAT_UNDO_STACK_SIZE);

  // Initialize storage of assertions to pass to mcsat.
  init_ivector(&s->mcsat_assertions, DEF_MCSAT_ASSERTION_VECTOR_SIZE);
...
/*
 * Free the model generated by build_model after a SAT result.
 */
...
void destroy_mcarith_solver(mcarith_solver_t* s) {
  ...
  delete_ivector(&s->mcsat_assertions);
...
void mcarith_check_var_size(mcarith_solver_t *solver) {
  ...
  a = &solver->var_terms;
  resize_ivector(a, solver->simplex.vtbl.size);

  size = a->size;
  new_size = solver->simplex.vtbl.nvars;
  assert(new_size < solver->simplex.vtbl.size);
  for (int32_t* p = a->data + size; p < a->data + new_size; ++p) {
    *p = NULL_TERM;
  }
  a->size = new_size;
}
...
/**
 * This sets the value of an index in the array to a value while growing the
 * array if needed (new elements are initialized to zero).
 */
static
void term_ivector_grow_set(ivector_t* a, uint32_t idx, int32_t val) {
  uint32_t size;
  size = a->size;
  if (size < idx+1) {
    for (int32_t* p = a->data + size; p != a->data + idx; ++p) {
      *p = NULL_TERM;
    }
    a->size = idx+1;
  }
...
/*
Return term associated with theory variable.
*/
static
term_t get_term(mcarith_solver_t* solver, thvar_t v) {
  ...
  assert(v < solver->var_terms.size);
  term_t t = solver->var_terms.data[v];
  if (t != NULL_TERM) {
  ...
  term_ivector_grow_set(&solver->var_terms, v, t);
  return t;
}
...
thvar_t mcarith_create_pprod(void *s, pprod_t *p, thvar_t *map) {
  ...
  assert(solver->var_terms.capacity > v);

  term_t t = pprod_term_from_buffer(terms, &b);
  term_ivector_grow_set(&solver->var_terms, v, t);
...
thvar_t mcarith_create_rdiv(void *s, thvar_t num, thvar_t den) {
  ...
  term_ivector_grow_set(&solver->var_terms, v, arith_rdiv(mcarith_solver_terms(solver), nt, dt));
...
/**
 * Create term equivalent to mcarith_assertion for MCSAT.
 */
static
term_t mk_assertion_pred(mcarith_solver_t *solver, mcarith_assertion_t* a) {
  term_t t;
  term_table_t* terms;
  rba_buffer_t b;
  rational_t br;
  ...
  switch (a->type) {
  case VAR_EQ:
    return arith_bineq_atom(terms, 
            get_term(solver, a->def.eq.lhs),
            get_term(solver, a->def.eq.rhs));
  case VAR_EQ0:
    return arith_eq_atom(terms, get_term(solver, a->def.var));
  case VAR_GE0:
  ...
  case POLY_GE0:
    ...
    return p;
  case ATOM_ASSERT:
    return get_atom(solver, a->def.atom);
  default:
    longjmp(*solver->simplex.env, INTERNAL_ERROR);
  }
}
...
fcheck_code_t final_check_conflict(mcarith_solver_t* solver) {
  ...
  for (size_t i = 0; i < acnt; ++i) {
    ...
    if (a->lit != null_literal) {
      assert(literal_value(solver->simplex.core, a->lit) == VAL_TRUE);
      ivector_push(conflict, not(a->lit));
    }
  }
...
fcheck_code_t mcarith_final_check(void* s) {
  ...
  for (size_t i = 0; i < solver->assertion_count; ++i) {
    ...
    ivector_push(&solver->mcsat_assertions, a->tt ? p : not_term(terms, p));
  }
...
void mcarith_backtrack(mcarith_solver_t *solver, uint32_t back_level) {
  ...
  (void)vc; // SUPPRESS unused variable warning in release builds.
  assert(vc == solver->simplex.vtbl.nvars);
}
...
void mcarith_pop(mcarith_solver_t *solver) {
  ...
  ivector_shrink(&solver->var_terms, solver->simplex.vtbl.nvars);
}
...
void mcarith_reset(mcarith_solver_t *solver) {
  ...
  assert(vc ==  solver->simplex.vtbl.nvars); // FIXME
...
bool mcarith_assert_atom(mcarith_solver_t *solver, void *atom_pointer, literal_t l) {
  ...
  assert(boolvar_of_atom(atbl->atoms + atom_index) == var_of(l));
