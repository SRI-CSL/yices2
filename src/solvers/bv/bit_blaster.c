...
/*
 * Add p * 2^k to sum and assert d = bit k of the result
 * - p must be a non-null literal
 * - sum must be an array of n literals
 */
static void bit_blaster_bvadd1(bit_blaster_t *s, literal_t *sum, literal_t p, uint32_t n, uint32_t k) {
   ...
   */
  for (i=k; i<n && p != false_literal; i++) {
    find_half_add(s, sum[i], p, &w, &c); // half-adder: w = p + sum[i], c = carry
    if (w == null_literal) {
      assert(c == null_literal);
      // create hald_add(sum[i], p) with output w and c
      w = bit_blaster_fresh_literal(s);
      c = bit_blaster_fresh_literal(s);
      make_half_add(s, sum[i], p, w, c);
    }
    sum[i] = w;
    p = c;
  }
...
/*
 * - add 2^k * (a[0].b[k] + a[1].b[k-1] + ... + a[k].b[0]) to sum
 *   then assert d = bit[k] of sum
 * - sum, a, and b must be arrays of n literals
 * - d must be a non-null pseudo literal
 * - k must satisfy (0 <= k < n)
 */
static void bit_blaster_column_product(bit_blaster_t *s, literal_t *sum, literal_t *a, literal_t *b, uint32_t n,
                                       literal_t d, uint32_t k) {
  remap_table_t *rmap;
  uint32_t i;
  literal_t p, f, w, c;
  ...
  // add 2^k * (a[0].b[k] + ... + a[k-1].b[1]) to sum
  for (i=0; i<k; i++) {
    p = make_and2(s, a[i], b[k-i]); // p = a[i].b[k-i]
    bit_blaster_bvadd1(s, sum, p, n, k);  // add p to sum
  }
  ...
  rmap = s->remap;
  f = remap_table_find(rmap, d);
  p = make_and2(s, a[k], b[0]);
  find_half_add(s, sum[k], p, &w, &c); // half-adder: w = p + sum[k], c = carry
  if (w == null_literal) {
    /*
     * half_add(sum[k], p) does not exist
     * create it with output (f, c)
     */
    assert(c == null_literal);
    c = bit_blaster_fresh_literal(s);
    if (f == null_literal) {
    ...
    // assert (f, c) = half-add(sum[k], p);
    make_half_add(s, sum[k], p, f, c);
    sum[k] = f;
  } else {
    /*
     * half_add(sum[k], p) does exist, with output (w, c)
     * w = sum, c = carry
     */
    // enforce d == w
    if (f == null_literal) {
      remap_table_assign(rmap, d, w);
    } else {
      bit_blaster_eq(s, f, w);
    }
    sum[k] = w;
  }
...
/*
 * Assert u = (bvmul a b): variant implementation
 * - a and b must be arrays of n literals
 * - u must be an array of n non-null pseudo literals
 */
void bit_blaster_make_bvmul2(bit_blaster_t *s, literal_t *a, literal_t *b, literal_t *u, uint32_t n) {
  ivector_t *v;
  literal_t *aux;
  uint32_t i;
  ...
  /*
   * aux stores the partial sums. It's initially 0b00000...
   */
  for (i=0; i<n; i++) {
    aux[i] = false_literal;
  }
  ...
  /*
   * Compute bit[i] of the product, assert u[i] = resulting bit
   * Store the carries in aux[i...n-1]
   */
  for (i=0; i<n; i++) {
    bit_blaster_column_product(s, aux, a, b, n, u[i], i);
  }
...
/*
 * Partial product: add (q * b * 2^k)  to a
 * - a and b must be arrays of n literals
 * - q must be a non-null literal
 * - k must satisfy (0 <= k && k < n)
 */
static void bit_blaster_partial_addmul(bit_blaster_t *s, literal_t *a, literal_t *b,
                                       literal_t q, uint32_t n, uint32_t k) {
  uint32_t i;
  literal_t p, s0, c0, d0;

  assert(0 <= k && k < n && q != null_literal);
  ...
  for (i=k; i<n; i++) {
    ...
    if (s0 == null_literal) {
      ...
      s0 = bit_blaster_fresh_literal(s);
      d0 = bit_blaster_fresh_literal(s);
      make_full_add(s, a[i], p, c0, s0, d0);
    }
    a[i] = s0;
    c0 = d0;
  }
...
/*
 * Compute r := r + b * q
 * - r, b, and q must be literal vectors of size n
 */
static void bit_blaster_addmul(bit_blaster_t *s, literal_t *r, literal_t *b, literal_t *q, uint32_t n) {
  uint32_t i;

  for (i=0; i<n; i++) {
    bit_blaster_partial_addmul(s, r, b, q[i], n, i);
  }
}


/*
 * Return l equivalent to (b == 0)
 * - b must be an array of n literals
 */
static inline literal_t make_bv_is_zero(bit_blaster_t *s, literal_t *b, uint32_t n) {
  return not(bit_blaster_make_or(s, n, b));
}
...
static int32_t bv_is_power_of_two(bit_blaster_t *s, literal_t *b, uint32_t n) {
  ...
  for (i=0; i<n; i++) {
    ...
    if (l == true_literal && k < 0) {
      k = i; // first bit equal to 1
...
/*
 * Assert q = quotient of a / 2^k (unsigned)
 * - a must be an array of n literals
 * - q must be an array of n pseudo literals
 */
static void bit_blaster_divpower_quotient(bit_blaster_t *s, literal_t *a, literal_t *q,
                                          uint32_t n, uint32_t k) {
  uint32_t i;

  assert(0 <= k && k < n);
  for (i=0; i<n-k; i++) {
    bit_blaster_pseudo_eq(s, q[i], a[i+k]);
  }
  for (i=n-k; i<n; i++) {
    bit_blaster_pseudo_eq(s, q[i], false_literal);
  }
}


/*
 * Assert r = remainder of a / 2^k unsigned
 * - a must be an array of n literals
 * - r must be an array of n pseudo-literals
 */
static void bit_blaster_divpower_remainder(bit_blaster_t *s, literal_t *a, literal_t *r,
                                           uint32_t n, uint32_t k) {
  uint32_t i;

  assert(0 <= k && k < n);
  for (i=0; i<k; i++) {
    bit_blaster_pseudo_eq(s, r[i], a[i]);
  }
  for (i=k; i<n; i++) {
    bit_blaster_pseudo_eq(s, r[i], false_literal);
  }
}


/*
 * Quotient in division by zero
 * - a must be an array of n literals
 * - q must be an array of n pseudo literals
 * We assert q = 0b111...11
 */
static void bit_blaster_divbyzero_quotient(bit_blaster_t *s, literal_t *a, literal_t *q, uint32_t n) {
  remap_table_t *rmap;
  literal_t f;
  uint32_t i;
  ...
  for (i=0; i<n; i++) {
    f = remap_table_find(rmap, q[i]);
    if (f == null_literal) {
      remap_table_assign(rmap, q[i], false_literal);
    } else {
      bit_blaster_eq(s, f, false_literal);
    }
  }
}
 ...
 * - r must be an array of n pseudo literals
 */
static inline void bit_blaster_divbyzero_remainder(bit_blaster_t *s, literal_t *a, literal_t *r, uint32_t n) {
  bit_blaster_pseudo_bveq(s, r, a, n);
}


/*
 * Initialize a from an array of n pseudo literals u
 * - if u[i] is mapped to null, create a fresh literal and assign it to u[i]
 */
static void init_from_pseudo_array(bit_blaster_t *s, literal_t *a, literal_t *u, uint32_t n) {
  remap_table_t *rmap;
  uint32_t i;
  literal_t f;
...
/*
 * Initialize array a with n fresh literals
 */
static void init_from_fresh(bit_blaster_t *s, literal_t *a, uint32_t n) {
  uint32_t i;

  for (i=0; i<n; i++) {
    a[i] = bit_blaster_fresh_literal(s);
  }
}
 ...
 * Assert q = (bvudiv a b) and r = (bvurem a b)
 */
void bit_blaster_make_udivision2(bit_blaster_t *s, literal_t *a, literal_t *b,
                                 literal_t *q, literal_t *r, uint32_t n) {
  ivector_t *v;
  literal_t *quotient;
  literal_t *remainder;
  ...
  k = bv_is_power_of_two(s, b, n);
  if (k >= 0) {
    // division by 2^k
    if (q != NULL) bit_blaster_divpower_quotient(s, a, q, n, k);
    if (r != NULL) bit_blaster_divpower_remainder(s, a, r, n, k);
    return;
  }

  // division by zero: quotient = 0b11..1, remainder = a
  l = make_bv_is_zero(s, b, n);
  if (l == true_literal) {
    if (q != NULL) bit_blaster_divbyzero_quotient(s, a, q, n);
    if (r != NULL) bit_blaster_divbyzero_remainder(s, a, r, n);
    return;
  }
  ...
  quotient = v->data;
  if (q != NULL) {
    init_from_pseudo_array(s, quotient, q, n);
  } else {
    init_from_fresh(s, quotient, n);
  }
  zero_extend_litarray(quotient, n);
  ...
  zero_extend_litarray(remainder, n);

  // copy b into aux_vector2 (zero extend)
  v = &s->aux_vector2;
  resize_ivector(v, 2 * n);
  ivector_reset(v);
  bit_blaster_zero_extend(v->data, b, n);
  b = v->data;

  // constraints on remainder: (b == 0) or (bvult r b)
  if (l == false_literal) {
    bit_blaster_assert_bvult(s, remainder, b, 2 * n);
  } else {
    p = bit_blaster_make_bvuge(s, remainder, b, 2 * n); // p = (r >= b)
    bit_blaster_assert_or2(s, l, not(p)); // assert b==0 or (r < b)
  }
  ...
  for (i=n; i<2*n; i++) {
    bit_blaster_eq(s, false_literal, remainder[i]);
  }
...
/*
 * SIGNED DIVISION: VERSION 2
 */

/*
 * Copy a[0...n-1] into aux[0...2n-1] with sign extension
 */
static void bit_blaster_sign_extend(literal_t *aux, literal_t *a, uint32_t n) {
  uint32_t i;
  literal_t l;
...
/*
 * Sign-extend a to size 2*n: set a[n ... 2n-1] to a[n-1]
 * - a must be an array of 2n literals
 */
static void sign_extend_litarray(literal_t *a, uint32_t n) {
  uint32_t i;
  literal_t l;
 ...
 * Assert q = (bvsdiv a b) and r = (bvsrem a b)
 */
void bit_blaster_make_sdivision2(bit_blaster_t *s, literal_t *a, literal_t *b,
                                 literal_t *q, literal_t *r, uint32_t n) {
  ivector_t *v;
  literal_t *quotient;
  literal_t *remainder;
  ...
  // initialize quotient: n fresh literals + sign extend
  v = &s->aux_vector3;
  resize_ivector(v, 2 * n);
  ivector_reset(v);
  quotient = v->data;
  ...
  sign_extend_litarray(quotient, n);

  // initialize remainder: n fresh literals + sign extend
  v = &s->aux_vector4;
  resize_ivector(v, 2 * n);
  ivector_reset(v);
  remainder = v->data;
  ...
  resize_ivector(v, 2 * n);
  ivector_reset(v);
  bit_blaster_sign_extend(v->data, b, n);
  b = v->data;
   ...
   */
  neg_b = make_bvneg_litarray(s, b, 2*n);
  if (l == false_literal) { // l <=> (b == 0)
    bit_blaster_assert_bvslt(s, remainder, b, 2 * n);     //  r < b
    bit_blaster_assert_bvslt(s, neg_b, remainder, 2 * n); // -b < r
  } else {
    p1 = bit_blaster_make_bvsge(s, remainder, b, 2 * n);     // p1 = (r >= b);
    p2 = bit_blaster_make_bvsge(s, neg_b, remainder, 2 * n); // p2 = (-b >= r);
    p1 = bit_blaster_make_or2(s, p1, p2);                    // not(p1) = (-b < r < b)
    bit_blaster_assert_or2(s, l, not(p1));
  }
  safe_free(neg_b);

  p1 = make_bv_is_zero(s, remainder, 2 * n);           // p1 := (r == 0)
  p2 = bit_blaster_make_eq(s, remainder[n-1], a[n-1]); // p2 := (sign_of(r) == sign_of(a))
  bit_blaster_assert_or2(s, p1, p2);

  // build r := bq + r
  bit_blaster_addmul(s, remainder, b, quotient, 2 * n);

  // assert (sign extend of a) = r
  for (i=0; i<n; i++) {
    bit_blaster_eq(s, a[i], remainder[i]);
  }
  l = a[n-1];
  for (i=n; i<2*n; i++) {
    bit_blaster_eq(s, l, remainder[i]);
  }
}
