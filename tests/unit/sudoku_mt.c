...
bool equal_solutions(game_t lhs_solution, game_t rhs_solution){
  int32_t i;
  int32_t j;

  for(i = 0; i < DIMENSION; i++){
    for(j = 0; j < DIMENSION; j++){
      if(lhs_solution[i][j] != rhs_solution[i][j]){ return false; }
    }
  }
...
yices_thread_result_t YICES_THREAD_ATTR sudoku_solver_thread(void* arg){
  ...
  FILE* output = tdata->output;
  {
    ...
    context_t *ctx;
    smt_status_t status;
    game_t *solution = (game_t *)tdata->extra;
    ...
    if(!assert_sudoku_rules(ctx, b)){
      fprintf(output, "Asserting Sudoku rules failed: %s\n", yices_error_string());
    }

    if(!pose_problem(ctx, b, test_game_1)){
      fprintf(output, "Asserting the initial conditions failed: %s\n", yices_error_string());
    }
    ...
    if(status == STATUS_SAT){
      ...
      if(get_solution(ctx, b,  *solution)){
        ...
        print_solution(output, *solution);
...
int main(int argc, char* argv[]) {
  ...
  if(argc != 2){
    ...
    if(nthreads < 0){
      ...
      /* each thread is going to compute a solution */
      game_t *nsolutions =  (game_t *)calloc(nthreads, sizeof(game_t));
    ...
    if(nthreads == 0){
      game_t solution;
      thread_data_t tdata = {0, stdout, &solution};
      sudoku_solver_thread(&tdata);
    } else {
      launch_threads(nthreads, nsolutions, sizeof(game_t), "sudoku_solver", sudoku_solver_thread, true);
    }


    //should check that all the solutions are the same.
