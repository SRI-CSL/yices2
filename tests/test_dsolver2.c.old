...
/*
 * Print v as a set of rows
 */
static void print_explanation(ivector_t *v) {
  uint32_t i, n;
...
static void assert_system(dsolver_t *solver, dsystem_t *s) {
  ...
  if (dsolver_status(solver) == DSOLVER_READY) {
    ...
    // solve the equations and print the solutions    
    ...
    if (feasible) {
      ...
      n = solver->nvars;
      for (i=1; i<n; i++) {
	if (dsolver_gen_sol(solver, i) != NULL) {
	  ivector_reset(&expl);
	  dsolver_explain_solution(solver, i, &expl);
	  printf("variable x_%"PRIu32": ", i);
	  print_explanation(&expl);
	}
      }
      printf("\n");

      np = dsolver_num_parameters(solver);
      init_mini_simplex(&splx);
      mini_simplex_set_eq(&splx, n, np, dsolver_get_gen_sol(solver));
      printf("\n*** Mini simplex: initial tableau ***\n");
      dsolver_print_tableau(stdout, &splx.matrix, n);
      printf("\n");      
      delete_mini_simplex(&splx);
