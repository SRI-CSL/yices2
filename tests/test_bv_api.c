...
static void print_status(smt_status_t status) {
  if (status >= STATUS_IDLE && status <= STATUS_ERROR) {
    printf("%s", status2string[status]);
  } else {
    printf("Invalid context status\n");
    printf("Please report this bug to yices-bugs@csl.sri.com\n");
    fflush(stdout);
    exit(1);
  }
}
 ...
 */
static void print_bvconst(uint32_t n, int32_t val[]) {
  printf("0b");
  while (n > 0) {
    n --;
 ...
 */
static void example1(context_t *ctx) {
  type_t bv4;
  term_t i4, i5, x;
  term_t b;
  term_t assertion[4];
  ...
  if (code < 0) {
    printf("Assertion failed with error code = %d\n", (int) yices_get_error_code());
    goto done;
  }
  ...
  printf("Status after assertions: ");
  print_status(status);
  printf("\n");
  ...
  if (status == STATUS_IDLE) {
    ...
    case STATUS_UNKNOWN:
      ...
      if (code < 0) {
	printf("Eval failed for 'x' with error code = %d\n", (int) yices_get_error_code());
      } else {
	printf("x is ");
	print_bvconst(4, buffer);
	printf("\n");
      }

      code = yices_eval_bv_term_in_model(model, i4, buffer);
      if (code < 0) {
	printf("Eval failed for 'i4' with error code = %d\n", (int) yices_get_error_code());
      } else {
	printf("i4 is ");
	print_bvconst(4, buffer);
	printf("\n");
      ...
      if (code < 0) {
	printf("Eval failed for 'i5' with error code = %d\n", (int) yices_get_error_code());
      } else {
	printf("i5 is ");
	print_bvconst(4, buffer);
	printf("\n");
      ...
      if (code < 0) {
	printf("Eval failed for 'i5 xor (i4 xor 0b0011)' with error code = %d\n", (int) yices_get_error_code());
      } else {
	printf("i5 xor (i4 xor 0b0011) is ");
	print_bvconst(4, buffer);
	printf("\n");
      }
...
static void example2(context_t *ctx) {
  type_t bv32;
