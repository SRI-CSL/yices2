...
The MCSAT solver supports several parameters that control its behavior for non-linear arithmetic and bit-vector solving.

+----------------------+-------------+----------------------------------------------+
| Parameter Name       | Type        | Meaning                                      |
+======================+=============+==============================================+
| mcsat-nra-mgcd       | Boolean     | Use model-based GCD instead of PSC for       |
|                      |             | projection in non-linear arithmetic          |
+----------------------+-------------+----------------------------------------------+
| mcsat-nra-nlsat      | Boolean     | Use NLSAT projection instead of Brown's      |
|                      |             | single-cell construction                     |
+----------------------+-------------+----------------------------------------------+
| mcsat-nra-bound      | Boolean     | Search by increasing the bound on variable   |
|                      |             | magnitude                                    |
+----------------------+-------------+----------------------------------------------+
| mcsat-nra-bound-min  | Integer     | Initial lower bound for variable magnitude   |
|                      |             | search (must be >= 0)                        |
+----------------------+-------------+----------------------------------------------+
| mcsat-nra-bound-max  | Integer     | Maximal bound for variable magnitude search  |
|                      |             | (must be >= 0)                               |
+----------------------+-------------+----------------------------------------------+
| mcsat-bv-var-size    | Integer     | Size of bit-vector variables in MCSAT search |
|                      |             | (must be >= 1)                               |
+----------------------+-------------+----------------------------------------------+
| mcsat-l2o            | Boolean     | Enable L2O mode for MCSAT solving            |
+----------------------+-------------+----------------------------------------------+

These parameters can be set using function :c:func:`yices_set_param`. For example:

.. code-block:: c

   yices_set_param("mcsat-nra-mgcd", "true");
   yices_set_param("mcsat-nra-bound", "true");
   yices_set_param("mcsat-nra-bound-min", "1");
   yices_set_param("mcsat-nra-bound-max", "100");

The parameters affect how MCSAT handles non-linear arithmetic and bit-vector constraints:

- ``mcsat-nra-mgcd``: When true, uses model-based GCD for projection in non-linear arithmetic solving
- ``mcsat-nra-nlsat``: When true, uses NLSAT projection instead of Brown's single-cell construction
- ``mcsat-nra-bound``: When true, enables search by increasing bounds on variable magnitudes
- ``mcsat-nra-bound-min``: Sets the initial lower bound for variable magnitude search
- ``mcsat-nra-bound-max``: Sets the maximum bound for variable magnitude search
- ``mcsat-bv-var-size``: Controls the size of bit-vector variables in MCSAT search
- ``mcsat-l2o``: When true, enables L2O mode for MCSAT solving
