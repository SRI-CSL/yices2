 ...
 *
 * Added array to keep statistics on decision variables
 * Configured to mimic: yices_smt --fast-restarts --randomness=0
 * + use tie-breaking in selecting decision literals.
 */


#ifndef __SAT_SOLVER10_H
#define __SAT_SOLVER10_H
...
typedef struct sat_solver_s {
  ...
  uint32_t *level;
  uint32_t *dcount;    // dcount[x] = number of times x was used as decision variable
...
/*
 * Set the initial activity of variable x to a
 * - a must be less than VAR_ACTIVITY_THRESHOLD
 */
extern void sat_solver_set_var_activity(sat_solver_t *solver, bvar_t x, double a);


/*
 * Read the decision count for variable x
 */
static inline uint32_t sat_solver_var_dcount(sat_solver_t *solver, bvar_t x) {
  assert(0 <= x && x < solver->nb_vars);
  return solver->dcount[x];
}
