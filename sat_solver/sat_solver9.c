 ...
 * 
 * TOOD: Fix clause deletion heuristics:
...
/*
 * Remove element at index i from the heap
...
void init_sat_solver(sat_solver_t *solver, uint32_t size) {
  ...
  solver->dcount = (uint32_t *) safe_malloc(size * sizeof(uint32_t));
...
static void sat_solver_extend(sat_solver_t *solver, uint32_t new_size) {
  ...
  solver->dcount = (uint32_t *) safe_realloc(solver->dcount, new_size * sizeof(uint32_t));
 ...
 */
void sat_solver_set_var_activity(sat_solver_t *solver, bvar_t x, double a) {
  assert(0 <= x && x < solver->nb_vars && a < VAR_ACTIVITY_THRESHOLD);
  heap_remove(&solver->heap, x);
  solver->heap.activity[x] = a;
