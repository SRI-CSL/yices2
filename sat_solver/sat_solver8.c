/*
 * Baseline SAT solver (Version 8)
 * - same data structures as version 2
 * - restart strategy similar to Minisat
 * - negative branching
 * - clause-deletion strategy similar to Minisat
 *
...
static clause_t *new_clause(uint32_t len, literal_t *lit) {
  ...
  result = (clause_t *) safe_malloc(sizeof(clause_t) + sizeof(literal_t) + len * sizeof(literal_t));
...
void init_sat_solver(sat_solver_t *solver, uint32_t size) {
  ...
  solver->end_watch = (link_t **) safe_malloc(lsize * sizeof(link_t *));
 ...
 */
static void reset_end_watch(sat_solver_t *solver) {
  uint32_t i, n;

  n = solver->nb_lits;
  for (i=0; i<n; i++) {
    if (solver->watch[i] == NULL_LINK) {
      solver->end_watch[i] = &solver->watch[i];
    }
  }
}
...
static void sat_solver_extend(sat_solver_t *solver, uint32_t new_size) {
  ...
  solver->end_watch = (link_t **) safe_realloc(solver->end_watch, lsize * sizeof(link_t *));
  reset_end_watch(solver);
...
void sat_solver_add_vars(sat_solver_t *solver, uint32_t n) {
  ...
  for (i=nvars; i<nvars+n; i++) {
    ...
    solver->end_watch[l0] = &solver->watch[l0];
    solver->watch[l1] = NULL_LINK;
    solver->end_watch[l1] = &solver->watch[l1];
  }
...
static void add_clause_core(sat_solver_t *solver, uint32_t n, literal_t *lit) {
  ...
  // set watch literals
  l = lit[0];
  *solver->end_watch[l] = mk_link(cl, 0);
  solver->end_watch[l] = &cl->link[0];

  l = lit[1];
  *solver->end_watch[l] = mk_link(cl, 1);
  solver->end_watch[l] = &cl->link[1];
...
static clause_t *add_learned_clause(sat_solver_t *solver, uint32_t n, literal_t *lit) {
  ...
  // add cl as last element to the watched lists
  l = lit[0];
  *solver->end_watch[l] = mk_link(cl, 0);
  solver->end_watch[l] = &cl->link[0];
 ...
 */
static inline void cleanup_watch_list(sat_solver_t *solver, literal_t l0) {
  link_t lnk;
  clause_t *cl;
  link_t *list;

  list = solver->watch + l0;
 ...
 * - l0 = literal
 */
static inline int propagation_via_watched_list(sat_solver_t *sol, uint8_t *val, literal_t l0) {
  bval_t v1;
  clause_t *cl;
  link_t *list;
  link_t link;
  ...
  while (link != NULL_LINK) {
	...
	link = cl->link[i];
	cl->link[i] = NULL_LINK;
	*sol->end_watch[l] = mk_link(cl, i);
	sol->end_watch[l] = &cl->link[i];
...
static int32_t propagation(sat_solver_t *sol) {
  ...
  for (i = sol->stack.prop_ptr; i < sol->stack.top; i++) {
    ...
    code = propagation_via_watched_list(sol, val, l);
    if (code != no_conflict) return code;
  }
