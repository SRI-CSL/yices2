...
/*
 * HACKS FOR EXPERIMENTING WITH VARIABLE ORDERING
 */

/*
 * Sort variables in decreasing order of decision count
 * - a = array of variables
 * - n = size of that array
 * - d = array of decision counts: d[x] = decision count for variable x
 */
static void qsort_variables(int32_t *a, uint32_t n, uint32_t *d);

// insertion sort
static void isort_variables(int32_t *a, uint32_t n, uint32_t *d) {
  uint32_t i, j, r;
  int32_t x, y;
...
static inline void sort_variables(int32_t *a, uint32_t n, uint32_t *d) {
  if (n <= 10) {
    isort_variables(a, n, d);
  } else {
    qsort_variables(a, n, d);
  }
} 
...
static void qsort_variables(int32_t *a, uint32_t n, uint32_t *d) {
  ...
  sort_variables(a, j, d);
  j ++;
  sort_variables(a + j, n - j, d);
}
...
/*
 * Build an array of all variables then sort it
 * - return the variable array in decreasing order
 */
static int32_t *get_sorted_variables(sat_solver_t *solver) {
  int32_t *a;
  uint32_t i, n;

  n = solver->nb_vars;
  a = (int32_t *) safe_malloc(n * sizeof(int32_t));
  for (i=0; i<n; i++) {
    a[i] = i;
  }
  sort_variables(a, n, solver->dcount);
  return a;
}


/*
 * Export the variable dcounts into a file
 * - each line of the file is a pair (variable_id, decision count)
 */
static void export_decision_counts(sat_solver_t *solver, char *filename) {
  FILE *f;
  int32_t *a, x;
  uint32_t i, n;
  ...
  if (f == NULL) {
    ...
    printf("Exporting variable order to file %s\n", filename);
    fflush(stdout);
    a = get_sorted_variables(solver);
    n = solver->nb_vars;
    for (i=0; i<n; i++) {
      x = a[i];
      fprintf(f, "%"PRId32" %"PRIu32"\n", x, sat_solver_var_dcount(solver, x));
    }
    safe_free(a);
    fclose(f);
  }
...
/*
 * Read a file of decision counts and assign them as initial variable activities
 */
static void import_decision_counts(sat_solver_t *solver, char *filename) {
  FILE *f;
  uint32_t i, n, d;
  int32_t x, code;
  ...
  if (f == NULL) {
    ...
    n = solver->nb_vars;
    printf("Importing variable order from %s\n", filename);
    fflush(stdout);
    for (i=0; i<n; i++) {
      code = fscanf(f, "%"SCNd32" %"SCNu32, &x, &d);
      if (code == EOF) {
	printf("Warning: missing data in file %s\n", filename);
	break;
      } else if (code != 2 || x < 0 || x >= n) {
	printf("Warning: invalid data in file %s\n", filename);
	break;
      } else {
	sat_solver_set_var_activity(solver, x, 1e30 * d);
      }
    }
    printf("Set activity for %"PRIu32" variables\n", i);
    fflush(stdout);
    fclose(f);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  if (argc < 2 || argc > 3) {
    ...
    fprintf(stderr, "    or %s <input file> <order file>\n", argv[0]);
    exit(1);
  }
  ...
  if (resu < 0) {
    ...
    if (argc == 3) {
      order_file = argv[2];
      import_decision_counts(&solver, order_file);
    }
    ...
    resu = solve(&solver);
    print_results();
    if (resu == status_unsat) {
      export_decision_counts(&solver, "vars.data");
    }
